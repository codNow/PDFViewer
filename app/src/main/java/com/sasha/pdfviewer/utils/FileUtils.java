...
public class FileUtils {
    ...
    public static ArrayList<ModelPdf> getModelPdf(Context context){
        ArrayList<ModelPdf> modelArrayList = new ArrayList<>();
        Uri uri = MediaStore.Files.getContentUri("external");
        String [] projection = {
                MediaStore.Files.FileColumns._ID,
                MediaStore.Files.FileColumns.DATA,
        ...
        if (cursor != null){
            while (cursor.moveToNext()){
                ...
                boolean isSelected = false;
                ModelPdf modelPdf = new ModelPdf(title);
                if (!subString.contains("RecentFile")) {
                    modelArrayList.add(modelPdf);
                }
